...
using Content.Shared._EinsteinEngines.Psionics.Passives;
...
namespace Content.Server._EinsteinEngines.Research.Oracle;
...
public sealed class OracleSystem : EntitySystem
{
    ...
    {
        // Comparing by reference works because prototypes are singletons.
        // Comparing by ID is also a robust alternative.
        return given == target;
    }
    ...
    private bool GetRandomTechProto(Entity<OracleComponent> oracle, [NotNullWhen(true)] out string? proto)
    {
        ...
        var database = _research.GetServerIds(oracle.Owner)
