...
public sealed partial class SensorInfo : BoxContainer
{
    ...
    public SensorInfo(AtmosSensorData data, string address)
    {
        ...
        SensorAddress.Title = Loc.GetString("air-alarm-ui-window-listing-title",
            ("address", _address),
        ...
        foreach (var (gas, amount) in data.Gases)
        {
            ...
            var fractionGas = data.TotalMoles > 0 ? amount / data.TotalMoles : 0f;

            data.GasThresholds.TryGetValue(gas, out var threshold);

            label.SetMarkup(Loc.GetString("air-alarm-ui-gases-indicator",
                ("gas", $"{gas}"),
                ("color", AirAlarmWindow.ColorForThreshold(fractionGas, threshold)),
                ("amount", $"{amount:0.####}"),
    ...
    public void ChangeData(AtmosSensorData data)
    {
        ...
        foreach (var (gas, control) in _gasThresholds)
        {
            ...
            var amount = data.Gases.TryGetValue(gas, out var m) ? m : 0f;

            control.UpdateThresholdData(threshold, data.TotalMoles > 0 ? amount / data.TotalMoles : 0f);
        }
    }
}
